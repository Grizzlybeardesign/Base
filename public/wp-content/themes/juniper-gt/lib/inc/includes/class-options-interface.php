...
class Options_Framework_Interface Extends Post_Meta_Inputs{
	...
	static function optionsframework_fields() {
		...
		if('grizzly_settings_import_and_export'===$option_name){
			// get the data for import and export
			$import_and_export = new Option_Import_And_Export();
		 	$settings['export_data'] = $import_and_export->export_data();	
		...
		$counter = 0;
		$menu = '';
		parent::grizzly_input_fields($options,$options_framework,$option_name,$settings,$counter,$menu,$allowedtags);
	...
	static function optionsframework_custom_fields($options) {
		...
		foreach ( $options as $value ) {
			...
			case 'text':
				...
				case 'range':
				$output .= '<input id="' . esc_attr( $value['id'] ) . '" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" type="range" min="' . esc_attr( $value['min'] ) . '" max="' . esc_attr( $value['max'] ) . '" step="' . esc_attr( $value['step']) . '"value="' . esc_attr( $val) . '"/><output class="range-value-display">'.esc_attr( $val).'</output>';
				break;
	...
	static function optionsframework_custom_fields_gallery($options) {
