 ...
 */
 ...
 public function gallery_meta_box(){
   	$exists = post_type_exists( 'gallery' );
        if($exists):
 	    add_meta_box(
        'gallery-images',
        __( 'Gallery', 'sitepoint' ),array($this,'gallery_meta_box_callback'),
        'gallery',
         'normal', 'default'
    );
        endif;
 ...
 public function gallery_meta_box_callback( $post ) {
	...
	?><ul class='image-preview-wrapper gallery-images sortable'>
	   ...
	   <?php 
	    ...
	    $attachment_url = wp_get_attachment_thumb_url( $attacment_id );

	   ?>
	   	   <li class="image" >	<img  src='<?php echo $attachment_url ?>' width='126' height='126' style='max-height: 126px; width: 126px;'>
	   	    <a href="javascript:void(0)" class="btn-gellery-remove" >Remove</a>
